...
class Model(torch.nn.Module):
    def __init__(self, config):
        ...
        # lstm 模型
        input_time = {'cat': 2, 'add': 1, 'sub': 1, 'mul': 1, 'mix': 5, 'max_pooling': 1}
...
#                            config.num_classes)  
        self.fc = nn.Linear(3842,
                            config.num_classes) 
        # 自定义全连接层 ，输入数（输入的最后一个维度），输出数（多分类数量），bert模型输出的最后一个维度是1024，这里的输入要和bert最后的输出统一
    ...
    def cat_features(self, text1, text2):
        ...
        fe = torch.cat((text1, text2, addition, subtract, multiple, cos, eu), dim=1)
        return fe
    def max_pooling(self, text1, text2):
        return functools.reduce(torch.max, (text1, text2))
